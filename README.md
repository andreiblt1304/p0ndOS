...
- Heap allocator carved out of a manually mapped virtual range using `linked_list_allocator::LockedHeap`, with a bump-allocator prototype for comparison (`src/allocator.rs`, `src/allocator/bump.rs`).
...
5. Execution drops into `hlt_loop`, using the CPU `hlt` instruction between interrupts to avoid busy spinning.
...
- `src/allocator.rs`: Heap mapping and `LockedHeap` setup; includes a `Dummy` allocator and a bump-allocator prototype in `allocator/bump.rs`.
...
- Swap allocators by replacing `LockedHeap` in `src/allocator.rs` with the bump allocator (or another) once it supports deallocation.
